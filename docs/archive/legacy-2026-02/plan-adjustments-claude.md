# Adjustments to Claude Plan for WE3 Website Experiment (Post-Conversation Baseline)
...
1. Update **Overview / Scope** to reflect: V0 is a mocked, deterministic brief builder (no API now). Keep the “rules engine MVP” framing, but remove any near-term API/consent work from MVP.
2. Add a **Tokens Pipeline** section: introduce `tokens/tokens.json` as source-of-truth, Style Dictionary build, and committed outputs (`tokens.generated.css` + `tokens.generated.ts`). Replace any mention of per-variant `tokens.css` with shared generated outputs.
3. Replace any styling guidance with: **vanilla CSS consuming generated variables**, light/dark only, aligned to Curated Collector Domestic.
4. Add **strict CSS isolation** instruction for Claude variant: all selectors prefixed under `[data-variant='claude']` (or equivalent).
5. Update **Pricing copy plan**: explicitly state fixed set of people (WE3) publicly, but **do not publish numeric weekly rate**; rate discussed after the brief.
6. Adjust **Case studies** section: keep gallery approach, but label as illustrative; forbid real client logos/brands.
7. Update **Analytics**: make optional/autonomous for Claude; ensure no PII.
8. Update **Acceptance/MVP**: your output formats can be novel, but must export (1) human-readable artifact and (2) JSON.
